...
Session_::Session_(boost::asio::io_service& _ioService)
		: socket_(_ioService)
{
...
Session_::~Session_(void)
{
}


void Session_::start(void)
{
	// 告诉链接成功的客户端，你想要的信息。
	...
	boost::asio::async_write(socket_, boost::asio::buffer(msg, strlen(msg)),
													 boost::bind(&Session_::init_handler, shared_from_this(),
																			 boost::asio::placeholders::error));
}

boost::asio::ip::tcp::socket& Session_::socket(void)
{
	return socket_;
}
...
// 第一个协议包
void Session_::init_handler(const boost::system::error_code& _error)
{
	if(_error)
	{
		return;
	...
	boost::asio::async_read(socket_, boost::asio::buffer(msg_, 10),
													boost::bind(&Session_::analyse_handler, shared_from_this(),
																			boost::asio::placeholders::error));
}

void Session_::analyse_handler(const boost::system::error_code& _error)
{
	if(_error)
	{
		return;
	...
	boost::asio::async_write(socket_, boost::asio::buffer(msg, REPLY_SIZE),
													 boost::bind(&Session_::write_handler, shared_from_this(),
																			 boost::asio::placeholders::error));
}


// 完成数据传输
void Session_::done_handler(const boost::system::error_code& _error)
{
	if(_error)
	{
		return;
...
void Session_::read_handler(const boost::system::error_code& _error, size_t _readSize)
{
	if(_error)
	{
		return;
...
void Session_::write_handler(const boost::system::error_code& _error)
{
	if(_error)
	{
		return;
	...
	boost::asio::async_read(socket_, boost::asio::buffer(msg_, maxSize_),
													boost::bind(&Session_::read_handler, shared_from_this(),
